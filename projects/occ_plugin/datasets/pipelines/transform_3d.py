...
class CustomCollect3D(object):
    ...
    def __call__(self, results):
        ...
        if 'occ_xyz' in results.keys():
            data['occ_xyz'] = results['occ_xyz']
        if 'occ_label' in results.keys():
            data['occ_label'] = results['occ_label']
        if 'occ_cam_mask' in results.keys():
            data['occ_cam_mask'] = results['occ_cam_mask']
...
class CustomOccCollect3D(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                            ...
                            'focal_positions', 'cam_positions', 'projection_mat',
                            'occ_xyz', 'occ_label', 'occ_cam_mask'
                            )):
        self.keys = keys
        self.meta_keys = meta_keys
